    ...
    while {true} do {
        ...
        if (_content != "") then {
            ...
            if (count _parts >= 5) then {
                ...
                private _zOffset = if (count _parts >= 6) then {parseNumber (_parts select 5)} else {0};

                diag_log format ["[TULEKASK] TULD! pos=[%1,%2] arv=%3 r=%4 z=%5", _x, _y, _count, _radius, _zOffset];
